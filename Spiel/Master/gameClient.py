...
class Game():    
    ...
    def setWeapons(self, posList):
        self.allWeapons.empty()
        positionList = self.convertStringsToPositions(posList)
        for pos in positionList:
            self.weaponList.append(pos)
            weapon = Weapon()
            weapon.rect.centerx = pos[0]
            weapon.rect.centery = pos[1]
        ...
        for player in self.allPlayers:
            for weapon in self.allWeapons:
                if player.rect.colliderect(weapon.rect):
                    ...
                    self.sendWeapon((weapon.rect.centerx,weapon.rect.centery))
