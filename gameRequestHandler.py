...
##                self.sendToClient(self.server.game.updatePosition(line.split[1]))
            if linde.upper().startswith("/UPDATEPOS"): #playerposition: Nickname, Position
                self.server.game.updatePlayerPosition(line.split()[1],line.split()[2])
            if line.upper().startswith("/SHOOT"): #schuss: Schussrichtung Spielername
                self.server.game.shoot(line.split()[1],line.split()[2], line.split()[3])
