...
class Imagebox(iconwidget.IconWidget):
    ...
    def __init__(self, x, y, width, height, icon = None, smooth = False):
        ...
        parameters:     int x-coordinate of the Imagebox (left)
                ...
                return values:  -
                """
        iconwidget.IconWidget.__init__(self, x, y, width, height, icon)
        self._smooth = smooth
    ...
    def _getAppearance(self, *args):
        ...
        """
        surface = iconwidget.IconWidget._getAppearance(self, *args)
        for tries in range(2):
            try:
                if self._smooth:
                    pygame.transform.smoothscale(self._icon, self._bounds.size, surface)
                else:
                    pygame.transform.scale(self._icon, self._bounds.size, surface)
                break
            except:
